...
export default function PracticeSession({ questions, subject, progress, onUpdateProgress, onFinish }: Props) {
  const practiceQuestions = questions.filter(q => q.subject === subject);
  ...
  const handleSelectOption = (index: number) => {
    ...
    if (!isCurrentlyCorrect && !newProgress.reviewList.includes(currentQ.id)) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 sm:p-6 bg-white rounded-2xl shadow-sm border border-orange-100">
      <div className="flex justify-between items-center mb-6">
        <span className="bg-orange-100 text-orange-800 px-4 py-1.5 rounded-full text-sm font-bold">
          第 {currentIndex + 1} 問 / 全 {practiceQuestions.length} 問
        </span>
      ...
      {!isAnswered && (
        <div className="mt-4 flex justify-end">
          <button 
            ...
            className="flex items-center text-blue-600 hover:text-blue-800 font-medium px-4 py-2 rounded-full hover:bg-blue-50 transition-colors"
          >
      ...
      {showHint && !isAnswered && (
        <div className="mt-4 bg-blue-50 p-4 rounded-xl border border-blue-100 animate-in fade-in zoom-in duration-200">
          <p className="flex items-start text-blue-800 font-medium whitespace-pre-wrap">
        ...
        <div className="mt-8 pt-8 border-t border-gray-100 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="mb-6">
            <h3 className="flex items-center text-lg font-bold mb-3 text-gray-800">
              <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm mr-3">解説</span>
            </h3>
            {earnedCoins !== null && (
          ...
          <button
            onClick={handleNext}
            className="w-full sm:w-auto sm:float-right flex items-center justify-center bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 px-8 rounded-full text-lg shadow-md transition-all active:scale-95"
          >
            {currentIndex < practiceQuestions.length - 1 ? '次の問題へ' : '練習をおわる'}
